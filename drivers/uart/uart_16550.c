...
static void u16550a_iowrite(struct uart_port *p, int offset, const int val)
{
...
static int u16550a_ioread(struct uart_port *p, int offset)
{
...
static void u16550a_putc(struct uart_port *p, const char c)
{
	u16550a_iowrite(p, UART_TX, c);

	while ((u16550a_ioread(p, UART_LSR) &
			(UART_LSR_THRE | UART_LSR_TEMT)) !=
			(UART_LSR_THRE | UART_LSR_TEMT))
		;
}

static void u16550a_puts(struct uart_port *p, const char *str, size_t count)
{
	size_t i = 0;
	...
	while ((str[i] != 0) &&	(i != count)) {
		if (str[i] == '\n')
			u16550a_putc(p, '\r');
		u16550a_putc(p, str[i++]);
	}

	spin_unlock_irqrestore(&p->lock, flags);
}

static ssize_t u16550a_gets(struct uart_port *p, char *buf, size_t count)
{
	size_t pos = 0;
	unsigned long flags = 0;
...
static void u16550a_setup(struct uart_port *p)
{
	...
	/* set DLAB bit */
	u16550a_iowrite(p, UART_LCR, 0x80);
	/* set baudrate divisor */
	u16550a_iowrite(p, UART_DLL, div & 0xFF);
	u16550a_iowrite(p, UART_DLM, (div >> 8) & 0xFF);
	/* clear DLAB; set 8 bits, no parity */
	u16550a_iowrite(p, UART_LCR, 0x3);
	/* enable FIFO mode, interrupt trigger by 14 bytes*/
	u16550a_iowrite(p, UART_FCR, 0xC7);
	/* no modem control DTR RTS */
	u16550a_iowrite(p, UART_MCR, 0x0B);
	/* set scratchpad */
	u16550a_iowrite(p, UART_SCR, 0x0);

	u16550a_ioread(p, UART_LSR);
	u16550a_ioread(p, UART_RX);
	u16550a_ioread(p, UART_IIR);
	u16550a_ioread(p, UART_MSR);

	spin_unlock_irqrestore(&p->lock, flags);
}

static void u16550a_attach(struct uart_port *p)
{
	u16550a_setup(p);
...
static void u16550a_detach(struct uart_port *p)
{
	u16550a_disable(p);
}

static void u16550a_suspend(struct uart_port *p)
{
	u16550a_disable(p);
}

static void u16550a_resume(struct uart_port *p)
{
	u16550a_attach(p);
}
...
static const struct uart_ops u16550a_port_ops = {
	...
	.detach = u16550a_detach,
};

static int __init u16550a_early_init(struct device_node *dn)
{
	struct uart_port *p = dev_get_drvdata(&dn->dev);

	u16550a_setup(p);

	p->ops = &u16550a_port_ops;

	return 0;
}
DECLARE_UART(ns16550a, "ns16550a", u16550a_early_init);
