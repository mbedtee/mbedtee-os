 ...
 * QEMU xlnx-zcu102 Cortex-A53x4 - Secondary CPUs PowerUp/PowerDown
 */
...
static int aarch64_cpu_up(unsigned int cpu)
{
...
static void aarch64_cpu_down(unsigned int cpu)
{
	udelay(5000);

	/*
	 * xlnx-zcu102 has the Power Control of Processor on/off
 ...
 */
static void aarch64_cpu_die(void)
{
	smc_call(2, 0, 0, 0);
}

static const struct cpu_pm_ops aarch64_pm_ops = {
	.cpu_up = aarch64_cpu_up,
	.cpu_down = aarch64_cpu_down,
	.cpu_die = aarch64_cpu_die
};
...
static int __init cpu_power_probe(struct device *dev)
{
	...
	iowritel(PA_OFFSET, phys_to_virt(0x00000210));
