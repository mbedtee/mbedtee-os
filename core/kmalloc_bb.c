...
static struct kmalloc_desc _buddy;
static struct kmalloc_desc _bitmap;
...
/*
 * Re-sort a buddy pool in the RB-tree when its key ordering
...
{
	unsigned int order = 0, old_key = 0;
	void *addr = NULL;
	unsigned long flags = 0;
	struct kmalloc_desc *d = buddy();
	struct kmalloc_buddy *p = NULL;
...
#define BMAP_RBKEY(b) ((((unsigned int)((b)->max)) << 16) + (b)->nbits + 1)
...
/*
 * Re-sort a bitmap pool in the RB-tree when its key ordering
 * relative to its neighbor is actually violated.
 * alloc decreases the key (check predecessor),
 * free increases the key (check successor).
 */
static inline void __kmalloc_bmap_rb_resort(struct kmalloc_bitmap *p,
	unsigned int old_key, struct rb_node **root)
{
	unsigned int new_key = BMAP_RBKEY(p);
	struct rb_node *n = NULL;

	if (new_key < old_key) {
		n = rb_prev(&p->node);
		if (n && new_key < BMAP_RBKEY(
				rb_entry_of(n, struct kmalloc_bitmap,
				node))) {
			rb_del(&p->node, root);
...
{
	void *addr = NULL;
	unsigned int idx = 0, max = 0, old_key = 0;
	unsigned long flags = 0;
	struct kmalloc_desc *d = bmap();
	struct kmalloc_bitmap *p = NULL;
	struct rb_node *n = NULL, *match = NULL;
...
again:
	if (p) {
		...
		*(unsigned int *)addr = nbits;
		...
		__kmalloc_bmap_rb_resort(p, old_key, &d->rbroot);
	}
...
void *kmalloc(size_t size)
{
	...
	if (nbits >= NBITS_USABLE_PER_PAGE) {
		...
		if ((alignedsize < BUDDY_NODE_SIZE - 24) ||
